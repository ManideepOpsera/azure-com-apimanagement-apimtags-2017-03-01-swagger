...
func Tagdescription_listbyapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/apis/%s/tagDescriptions%s", cfg.BaseURL, resourceGroupName, serviceName, apiId, subscriptionId, queryString)
		...
		var result models.TagDescriptionCollection
...
func CreateTagdescription_listbyapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_apis_apiId_tagDescriptions",
		mcp.WithDescription("Lists all Tags descriptions in scope of API. Model similar to swagger - tagDescription is defined on API level but tag may be assigned to the Operations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tagdescription_listbyapiHandler(cfg),
	}
}
