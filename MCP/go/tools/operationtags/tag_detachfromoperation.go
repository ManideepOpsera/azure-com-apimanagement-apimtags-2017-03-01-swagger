...
func Tag_detachfromoperationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/apis/%s/operations/%s/tags/%s%s", cfg.BaseURL, resourceGroupName, serviceName, apiId, operationId, tagId, subscriptionId, queryString)
...
func CreateTag_detachfromoperationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_apis_apiId_operations_operationId_tags_tagId",
		mcp.WithDescription("Detach the tag from the Operation."),
		...
		mcp.WithString("operationId", mcp.Required(), mcp.Description("Operation identifier within an API. Must be unique in the current API Management service instance.")),
		...
		mcp.WithString("If-Match", mcp.Required(), mcp.Description("The entity state (Etag) version of the Api schema to update. A value of \"*\" can be used for If-Match to unconditionally apply the operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tag_detachfromoperationHandler(cfg),
	}
}
