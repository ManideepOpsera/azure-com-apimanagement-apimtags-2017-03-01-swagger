...
func Tag_detachfromproductHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/products/%s/tags/%s%s", cfg.BaseURL, resourceGroupName, serviceName, productId, tagId, subscriptionId, queryString)
...
func CreateTag_detachfromproductTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_products_productId_tags_tagId",
		mcp.WithDescription("Detach the tag from the Product."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tag_detachfromproductHandler(cfg),
	}
}
