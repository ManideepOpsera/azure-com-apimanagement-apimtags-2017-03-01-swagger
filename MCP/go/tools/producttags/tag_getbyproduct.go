...
func Tag_getbyproductHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTag_getbyproductTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_products_productId_tags_tagId",
		mcp.WithDescription("Get tag associated with the Product."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tag_getbyproductHandler(cfg),
	}
}
