...
func Tag_assigntoproductHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTag_assigntoproductTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_products_productId_tags_tagId",
		mcp.WithDescription("Assign tag to the Product."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tag_assigntoproductHandler(cfg),
	}
}
