...
func Tag_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.TagCreateUpdateParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/tags/%s%s", cfg.BaseURL, resourceGroupName, serviceName, tagId, subscriptionId, queryString)
...
func CreateTag_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_tags_tagId",
		mcp.WithDescription("Updates the details of the tag specified by its identifier."),
		...
		mcp.WithString("If-Match", mcp.Required(), mcp.Description("ETag of the Tag Entity. ETag should match the current entity state from the header response of the GET request or it should be * for unconditional update.")),
		...
		mcp.WithObject("properties", mcp.Description("Input parameter: Tag contract Properties.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Tag_updateHandler(cfg),
	}
}
