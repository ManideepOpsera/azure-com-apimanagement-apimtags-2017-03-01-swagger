...
func Tag_listbyapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/apis/%s/tags%s", cfg.BaseURL, resourceGroupName, serviceName, apiId, subscriptionId, queryString)
...
func CreateTag_listbyapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_apis_apiId_tags",
		mcp.WithDescription("Lists all Tags associated with the API."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tag_listbyapiHandler(cfg),
	}
}
